...
# Whisper calls ffmpeg directly via subprocess, using whatever's on the
# system PATH — it does NOT use pydub's AudioSegment.converter setting.
# This must run before Whisper (or anything that imports it) is used,
# so it's placed as the very first thing in the entry point.
#
# Locally on Windows, ffmpeg usually isn't on PATH, so we add it manually.
# On Streamlit Community Cloud (Linux), ffmpeg is installed via
# packages.txt and is already on PATH — shutil.which finds it, so we skip
# the hardcoded Windows path there.
